...
public class MovieDao extends AbstractMFlixDao {
	...
	public List<Document> getMoviesByCountry(String... country) {
		...
		Bson projection = new Document("title", 1); // ("title", 1L)
...
//T1 - DONE
		...
		queryFilter = Filters.all("countries", Arrays.asList(country));
		...
		// Méthode suggerée par MongoDBUniversity:
...
//T4 - DONE: le test unitaire ne passe pas, mais le test en ligne si!
		moviesCollection
			.find(castFilter)
			.skip(skip)
			.limit(limit)
